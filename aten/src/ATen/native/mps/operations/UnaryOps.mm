...
static const char* ERFINV_OPS_TEMPLATE = R"METAL(
 ...
 kernel void erfinv_mps_kernel(constant uint& length [[buffer(0)]],
                         device {0}  *output [[buffer(1)]],
                         device {1}  *input [[buffer(2)]],
...
TORCH_IMPL_FUNC(erfinv_out_mps)(const Tensor& self, const Tensor& output_) {
  ...
  if (self.dim() == 0) {
    // handle scalar case on the CPU
    // return erfinv_op_cpu(self.item<float>());
  ...
  id<MTLComputePipelineState> cplState =
      getCPLState(device, getMetalType(output), getMetalType(self), getMetalType(self), "erfinv_mps_kernel");
  dispatch_sync(stream->queue(), ^() {
    ...
    id<MTLBuffer> outBuf = mps::getMTLBufferStorage(output);
    id<MTLBuffer> selfBuf = mps::getMTLBufferStorage(self);

    [commandEncoder pushDebugGroup:@"Dispatch erfinv_mps_kernel"];
